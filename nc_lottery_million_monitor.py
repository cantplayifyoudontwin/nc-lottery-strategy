...
class GameData:
    ...
    def get_mid_tiers(self) -> List[PrizeTier]:
        return [t for t in self.prize_tiers if t.is_mid_tier]
    
    def get_low_tiers(self) -> List[PrizeTier]:
        return [t for t in self.prize_tiers if t.is_low_tier]
    ...
    def calculate_mid_tier_health(self) -> Tuple[int, int, float]:
        """Returns (remaining, total, percent) for $500-$10K prizes"""
        mid_tiers = self.get_mid_tiers()
        if not mid_tiers:
            return 0, 0, 0.0
        total = sum(t.total for t in mid_tiers)
        remaining = sum(t.remaining for t in mid_tiers)
        pct = (remaining / total * 100) if total > 0 else 0.0
        return remaining, total, pct
...
def generate_html_report(games: List[GameData]) -> str:
    ...
    def generate_game_card(game: GameData, score: float, rank: int = None) -> str:
        ...
        mid_rem, mid_tot, mid_pct = game.calculate_mid_tier_health()
        bottom_pct = game.calculate_bottom_depletion()
        diff = game.calculate_differential()
        days = game.days_since_launch()
        ...
        mid_color = "#00FF88" if mid_pct >= 70 else "#FFD700" if mid_pct >= 50 else "#FF6B6B"
        ...
        <div class="game-card">
            ...
            <div class="metrics-grid">
                ...
                <div class="metric">
                    <div class="metric-label">MID-TIER ($500-$10K)</div>
                    <div class="metric-value" style="color: {mid_color}">{mid_pct:.0f}% remaining</div>
                    <div class="metric-sub">Loss minimization potential</div>
                </div>
...
<body>
    <div class="container">
        ...
        <div class="info-box">
            ...
            <p><strong>Score</strong> combines: Million prize health (40%), Mid-tier health (30%), Overall differential (20%), and Game freshness (10%).</p>
